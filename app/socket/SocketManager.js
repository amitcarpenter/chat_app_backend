...
class SocketManager {
  ...
  initializeSocketEvents() {
    SocketManager.io.on("connection", async (socket) => {
      ...
      socket.on("message-delivered", ({ messageId, receiverId }) => {
        if (SocketManager.connectedUsers[receiverId]) {
          SocketManager.io.to(SocketManager.connectedUsers[receiverId]).emit("message-delivered", { messageId });
        }
      });

      socket.on("message-read", ({ messageId, receiverId }) => {
        if (SocketManager.connectedUsers[receiverId]) {
          SocketManager.io.to(SocketManager.connectedUsers[receiverId]).emit("message-read", { messageId });
        }
      });

      socket.on("typing", ({ receiverId, senderId, isTyping }) => {
        if (SocketManager.connectedUsers[receiverId]) {
          SocketManager.io.to(SocketManager.connectedUsers[receiverId]).emit("typing", { senderId, isTyping });
        }
      });

      socket.on("attachment", ({ receiverId, attachment, senderId }) => {
        const message = {
          _id: Date.now(),
          attachment,
          createdAt: new Date(),
        ...
        if (SocketManager.connectedUsers[receiverId]) {
          SocketManager.io.to(SocketManager.connectedUsers[receiverId]).emit("attachment", message);
        }
      });
  ...
  sendMessageToUser(receiverId, message, roomId, senderId) {
  ...
  async sendMessageToBot(receiverId, message, roomId, senderId) {
    ...
    if (senderSocketId) {
      SocketManager.io.to(senderSocketId).emit("private-message", sendMessage);
    }
    ...
    try {
      ...
      const botResponseData = botResponse.data.output;
      if (botResponseData) {
        let message_id = message._id + 2;
        const botMessage = [{
          _id: `${message_id}`,
          ...
          userId: receiverId,
          image: "",
          user: { _id: receiverId, avatar: message.user.avatar, name: message.user.name },
        }];
